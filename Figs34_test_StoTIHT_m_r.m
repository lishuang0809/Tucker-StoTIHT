...
% This code is used to reproduce Figures 3 and 4. You may need to adjust the
...
for tt = 1:Trial
    ...
    for ri = 1:size(r_all,1)
        ...
        for mi = 1:length(m_all)
            ...
            if rel_err <= 1e-5
                count_TIHT(ri,mi) = count_TIHT(ri,mi) + 1;
            end
            ...
            rel_err = norm(vec(X)-XT)/norm(vec(X));
            if rel_err <= 1e-5
                count_StoTIHT(ri,mi) = count_StoTIHT(ri,mi) + 1;
            end
...
ylabel('Percentage of successful recovery','Interpreter','LaTex','FontSize',fs)
set(h,'Interpreter','LaTex','FontSize',fs,'location','southwest')
set(gca,'fontsize',fs)
set(gcf, 'Color', 'w');
axis tight;
...
xlabel('$m$','Interpreter','LaTex','FontSize',fs)
ylabel('Percentage of successful recovery','Interpreter','LaTex','FontSize',fs)
set(h,'Interpreter','LaTex','FontSize',fs,'location','southeast')
set(gca,'fontsize',fs)
set(gcf, 'Color', 'w');
%axis tight;
%axis([240 460 0 1])
